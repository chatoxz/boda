...
<div class="<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>-view container-fluid">
    ...
    <div class="row">
        <div class="col-md-6 <?= ($generator->saveAsNew) ? " col-sm-8" : " col-sm-9";?>">
            <h2><?= "<?= " ?><?= $generator->generateString(Inflector::camel2words(StringHelper::basename($generator->modelClass))) ?>.' '. Html::encode($this->title) ?></h2>
        </div>
        <div class="col-md-5<?= ($generator->saveAsNew) ? " col-sm-4" : " col-sm-3";?>" style="margin: 15px">
...
<?= "            <?= Html::a(" . $generator->generateString('Save As New') . ", ['save-as-new', " . $generator->generateUrlParams() . "], 
['class' => 'btn btn-info add-modal-data', 'title' => " . $generator->generateString('Save As New') . ", 'style' => 'margin-bottom:15px' ]) ?>" ?>
<?php endif;?>
            <?= "
            <?= Html::a(" . $generator->generateString('Update') . ", ['update', " . $generator->generateUrlParams() . "], 
            ['class' => 'btn btn-primary add-modal-data', 'title' => " . $generator->generateString('Update') . ", 'style' => 'margin-bottom:15px']) ?>
